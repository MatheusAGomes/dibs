...
class MeusIngressosScreen extends StatefulWidget {
  ...
  MeusIngressosScreen({required this.listaDeIngressos});
...
class _MeusIngressosScreenState extends State<MeusIngressosScreen> {
  ...
  Widget build(BuildContext context) {
    List<Widget> widgets = widget.listaDeIngressos!.map((item) {
      if (item.valid) {
        return Padding(
              ...
              value == 0
                  ? Container(
                      ...
                      child: ListView.builder(
                        ...
                        itemCount: widget.listaDeIngressos!.length,
                        itemBuilder: (context, index) {
                          if (widget.listaDeIngressos![index].valid)
                            return Padding(
                              padding: const EdgeInsets.symmetric(vertical: 10),
                              child: BannerMeuIngresso(
                                id: widget.listaDeIngressos![index].id,
                                local: widget.listaDeIngressos![index].address,
                                ativo: widget.listaDeIngressos![index].valid,
                                empresa: false,
                                anuncio: false,
                                image: NetworkImage(widget
                                        .listaDeIngressos[index].picture ??
                                    'https://firebasestorage.googleapis.com/v0/b/dibs-67d1a.appspot.com/o/PericlesEx.png?alt=media&token=72406ea1-1582-4ad4-af59-5b2df066cb99&_gl=1*ly3jbo*_ga*MTczOTA4MjMwNi4xNjk4MTgwNTUy*_ga_CW55HF8NVT*MTY5ODk0MjAwNS40LjEuMTY5ODk0NjExNS41NS4wLjA'),
                                titulo:
                                    widget.listaDeIngressos![index].eventName,
                                data: widget.listaDeIngressos![index].startDate,
                                hora: widget.listaDeIngressos![index].time!,
                                lote: '1° Lote',
                                tipo: 'Meia-Entrada',
                                corBanner: Colors.green,
                                corDoLote: Colors.green.shade900,
